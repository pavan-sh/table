...
describe('#getGroupedRowModel', () => {
  // This is a performance regression test that can be noisy on shared CI runners.
  // Run locally/explicitly with: TANSTACK_PERF_TESTS=1 pnpm test
  it.runIf(Boolean(process.env.TANSTACK_PERF_TESTS))(
    'groups 50k rows and 3 grouped columns with clustered data in less than 5 seconds',
    () => {
